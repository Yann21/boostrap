...
if [ -z "$JDK" ] && [ -s "$HOME/.PyCharm2019.3/config/pycharm.jdk" ]; then
  USER_JRE=$("$CAT" "$HOME/.PyCharm2019.3/config/pycharm.jdk")
  if [ ! -d "$USER_JRE" ]; then
...
elif [ -r "$HOME/.PyCharm2019.3/config/pycharm$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.PyCharm2019.3/config/pycharm$BITS.vmoptions"
